...
}else if(logical_tag == 'pearson'){
  for(i in 1:length(sample_name_vactor)){
    if(distinct_pearson_p_value_vector[i]<distinct_PValue_pearson_cutoff){
      this_type = 'GeneExpressionOutlier'
    }else{
      this_type = 'MainPopulationCell'
    }
...
}else if(logical_tag == 'spearman'){
  for(i in 1:length(sample_name_vactor)){
    if(distinct_spearman_p_value_vector[i]<distinct_PValue_spearman_cutoff){
      this_type = 'GeneExpressionOutlier'
    }else{
      this_type = 'MainPopulationCell'
    }
...
}else if(logical_tag == 'mi'){
  for(i in 1:length(sample_name_vactor)){
    if(distinct_mi_p_value_vector[i]<distinct_PValue_mi_cutoff){
      this_type = 'GeneExpressionOutlier'
    }else{
      this_type = 'MainPopulationCell'
    }
...
out_matrix = data.frame(cbind(merged_sample_type_to_qc[,-c(3,7)],mapped_reads_quantile_in_all_vector,mapping_rate_quantile_in_all_vector,library_complexity_quantile_in_all_vector))
header = c('Quantile.MappedReads','Quantile.MappingRate','Quantile.LibraryComplexity')
#header = c(colnames(merged_sample_type_to_qc),header)
header = c(colnames(merged_sample_type_to_qc)[-c(3,7)],header)

#out_matrix = rbind(header,out_matrix)
colnames(out_matrix) = header

outfile_name = paste(outfile_folder,'Samples_Seq_Quality_in_all_Quantile.txt',sep = '')
write.table(x = out_matrix,file = outfile_name,sep = '\t',quote = F,row.names = F,col.names = T)
